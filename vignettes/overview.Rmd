...
## dbMatrix
`dbMatrix` is a core package in the [`dbverse`](https://ed2uiz.github.io/dbverse/) library that inherits from the `dbData` base class and consists of two subclasses, `dbSparseMatrix` and `dbDenseMatrix`.
...
and `x` numerical vectors. `i` and `j` refer to the row and column indices, respectively, with specific `x` values or counts. Note: unlike the rest of R, the `i` and `j` vectors are zero-indexed following conventions from the `Matrix` R package for the `dgCMatrix` and `TSparseMatrix` class.
